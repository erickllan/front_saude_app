...
export default function HomeScreen() {
  ...
  useEffect(function () {
    Location.requestForegroundPermissionsAsync().then(function (resultado) {
      ...
      Location.getCurrentPositionAsync({}).then(function (local) {
        ...
        fetch('http://192.168.100.70:3000/unidades-saude')
  ...
  return (
    <View style={{ flex: 1, padding: 15, backgroundColor: '#c3ddde', marginTop: 20 }}>

      {/* Botão Histórico */}
      <TouchableOpacity
        style={{ backgroundColor: '#35ddd2', padding: 15, borderRadius: 10, marginBottom: 15 }}
        onPress={function () { router.push('/historico'); }}
      >
      ...
      {/* Input de Pesquisa */}
      <View style={{ backgroundColor: 'white', borderRadius: 8, padding: 5, marginBottom: 10, borderWidth: 1, borderColor: '#14c9cf', flexDirection: 'row', alignItems: 'center' }}>
          ...
          onChangeText={function (textoDigitado) {
            ...
            if (textoDigitado == '') {
              ...
              let arrayTemporario = unidades.filter(function (item: any) {
                let tudoJunto = (item.nome + ' ' + item.bairro + ' ' + item.tipo).toLowerCase();
              ...
              fetch('http://192.168.100.70:3000/localizacao', {
                method: 'POST',
              ...
              }).catch(function (e) { console.log('erro ao salvar'); });
            }
          }}
        />
      </View>
      ...
      <FlatList ////FlatList renderiza dinamicamente as unidades de saúde filtradas.
        ...
        renderItem={function (elemento: any) {
          ...
          if (item.tipo == 'Testagem HIV/IST/AIDS - Gratuito') corFundoTag = '#9a1b1b';

          return (
            <View style={{ backgroundColor: 'white', padding: 15, marginBottom: 10, borderRadius: 8, borderLeftWidth: 5, borderLeftColor: corFundoTag }}>
              
              {/* Tag do Tipo */}
              <View style={{ backgroundColor: corFundoTag, padding: 4, borderRadius: 5, alignSelf: 'flex-start', marginBottom: 5 }}>
                <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 10 }}>{item.tipo}</Text>
              </View>

              {/* Informações do Item */}
              <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{item.nome}</Text>
              <Text style={{ color: '#1f1f1f' }}>Endereço: {item.endereco} - {item.bairro}</Text>
              ...
              <Text style={{ color: 'blue', marginTop: 5 }}>Atendimento: {item.especialidade}</Text>
            </View>
          );
        }}
      />
